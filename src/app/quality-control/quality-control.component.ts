...
export class QualityControlComponent implements OnInit {
  ...
  createAndPrintLicensePlate() {
    ...
    this.error = null;
    this.licensePlateService.createLicensePlate(this.stateService.printer).subscribe(
      data => {
  ...
  scanLicensePlate() {
    ...
    this.licensePlateService.getLicensePlate(this.licensePlateId).subscribe(
      data => {
        if (data.status == 200) {
          this.router.navigate(["quality-control-scan"], {
            queryParams: { licensePlateId: this.licensePlateId }
          });
        }
